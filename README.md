...
The above image is a visual representation of the socioscapes  {scape} object. Additionally, the API also defines the following classes: [state], [layer], and [view]. Each class instance includes a {meta} member that provides information unique to the instance. Since the {scape} object is itself just an instance of the [scape] class, it also has a {meta} member. The [state] array stores instances of the [state] class, which are conceptualized as the complete contents of the DOM at a particular moment -- its state. Each {state} instance includes all of the data and configuration that are necessary to reproduce its corresponding DOM state (such as maps, charts, graphs, and other visualizations). This structure is meant to aid comparative and time series visualizations. For instance, suppose a user wishes to show a change in the way in which income is distributed across their city. They could create several {state} instances, one for each point in the time series. This allows for maximum data portability because each {state} is entirely independent of the others and can be edited, shared, and saved separately. Besides a map, the user may also wish to include charts, graphs, and tables. Socioscapes approaches this task by sub dividing each {state} into {layer} elements, which are static collections of raw numerical and geometric data, and {view} elements, which are DOM-rendered instances of such layers. Views are directly incorporated into the DOM whereas layers are dumb datastores. A chart contained within a DOM \<div\> container constitutes a socioscapes view but that same data from the same layer could also be used in many different maps, charts, graphs, and tables. Since this is a common GIS scenario, both the socioscapes API and the datastructure have been designed to facilitate it. The above schema is not set in stone and users can use the socioscapes.fn.extend function to edit, remove, and add classes as they see fit. To get a feel for the API, jump into the code or read through the following examples:
