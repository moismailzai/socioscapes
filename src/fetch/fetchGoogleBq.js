 ...
 */
export default function fetchGoogleBq(scapeObject, config) {
    config = config || {};
    let callback = newCallback(arguments),
        ...
        csvEntry,
        csvEntries = [],
        featureIdProperty = (config && config.featureIdProperty) ?
            config.featureIdProperty.toLowerCase() :
            'dauid',
        indexOfFeatureProperty = 1,
        indexOfValueProperty = 0,
        normalizationIdProperty = (config && config.normalizationIdProperty) ?
            config.normalizationIdProperty.toLowerCase() :
            'none',
        ...
        valueIdProperty = (config && config.valueIdProperty) ?
            config.valueIdProperty.toLowerCase() :
            'total',
        gapiConfig = {
            auth: {
    ...
    if (authToken) {
        ...
        gapi.client.load(gapiConfig.client.name, gapiConfig.client.version,
            function() {
    ...
    queryFetcher = function() {
        ...
        queryRequest.execute(function(result) {
            ...
            for (let i = 0; i < result.schema.fields.length; i++) {
                queryResult.meta.columns.push(
                    result.schema.fields[i].name.toLowerCase());
                queryResult.byColumn[result.schema.fields[i].name.toLowerCase()] = [];
                indexOfFeatureProperty = (result.schema.fields[i].name.toLowerCase() ===
                    featureIdProperty) ? i : indexOfFeatureProperty;
                indexOfValueProperty = (result.schema.fields[i].name.toLowerCase() ===
                    valueIdProperty) ? i : indexOfValueProperty;
            }
            result.rows.forEach(function(row) {
                for (let i = 0, parsedRow = {}; i < row.f.length; i++) {
                    ...
                    queryResult.byColumn[result.schema.fields[i].name].push(
                        row.f[i].v);
                }
                csvEntry = [];
                for (let field in parsedRow) { // for each column
                ...
                queryResult.geoJson.features.push(
                    {'type': 'Feature', 'properties': parsedRow});
            ...
            try {
                queryResult.geostats = new geostats(
                    queryResult.byColumn[valueIdProperty]);
            } catch {
                queryResult.geostats = null;
                console.log('geostats initialization failed.')
            }
